...
class Votes(commands.Cog):
    ...
    async def voter(self, interaction: discord.Interaction, brawler: str, tier: str):
        ...
        # Vérifier que le brawler existe
        found = None
        for key in brawlers:
        ...
        embed = discord.Embed(
            title=f"✅ Vote enregistré !",
            color=0x2ECC71
        )
        embed.add_field(
            name=f"{brawler_info['emoji']} {found}",
            value=f"Ton vote : **Tier {tier}** {tier_emoji(tier)}",
            inline=False
        )
        if ancien_vote:
    ...
    async def tierlist_communautaire(self, interaction: discord.Interaction):
        ...
        if not guild_votes:
            await interaction.response.send_message(
                "❌ Aucun vote enregistré ! Utilisez `/voter` pour commencer.",
                ephemeral=True
            )
            return
        ...
        embed = discord.Embed(
            title="🗳️ Tier List Communautaire",
            description=f"Basée sur les votes des membres de **{interaction.guild.name}**",
            color=0x9B59B6
        )
    ...
    async def mon_vote(self, interaction: discord.Interaction):
        ...
        if not mes_votes:
            await interaction.response.send_message(
                "❌ Tu n'as encore voté pour aucun brawler ! Utilise `/voter`.",
                ephemeral=True
            )
            return

        embed = discord.Embed(
            title=f"🗳️ Votes de {interaction.user.display_name}",
            description="\n".join(mes_votes),
            color=0x3498DB
        )
    ...
    async def reset_votes(self, interaction: discord.Interaction):
        ...
        if guild_id in votes:
            del votes[guild_id]
            save_votes(votes)
        await interaction.response.send_message("✅ Tous les votes ont été réinitialisés !", ephemeral=False)
    ...
    async def votes_brawler(self, interaction: discord.Interaction, brawler: str):
        ...
        if not brawler_votes:
            await interaction.response.send_message(
                f"❌ Aucun vote pour **{found}** ! Utilise `/voter {found}`.",
                ephemeral=True
            )
            return
        ...
        nb_votes = len(brawler_votes)

        embed = discord.Embed(
            title=f"{info['emoji']} Votes — {found}",
            color=0xFFD700
        )
        embed.add_field(
            name="📊 Tier communautaire",
            value=f"**{tier_communautaire}** {tier_emoji(tier_communautaire)}",
            inline=True
        )
        embed.add_field(name="👥 Total votes", value=str(nb_votes), inline=True)

        distribution = "\n".join([
            f"{tier_emoji(t)} Tier {t} : **{n}** vote{'s' if n > 1 else ''}"
            for t, n in tier_count.items() if n > 0
        ])
